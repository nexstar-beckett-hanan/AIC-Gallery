...
describe('Artwork Component', () => {
		it('shows 2 headings', async () => {
			const result = renderWithClient(
				<BrowserRouter>
					<Artwork artwork={ artwork } configUrl= { configUrl } page={ page }/>
				</BrowserRouter>
			);
		...
		it('has alt text if an image is present', async () => {
			...
			expect(result.getByAltText(/Bright landscape painting featuring spindly trees with lush leaves surrounded by light-green foliage, a small town visible between the trees at center, and a calm blue sea in the background\./i)).toBeInTheDocument();
		});
